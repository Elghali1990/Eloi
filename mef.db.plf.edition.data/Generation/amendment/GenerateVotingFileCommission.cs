...
public class GenerateVotingFileCommission(IViewRenderService _viewRenderService, IUnitOfWork _unitOfWork)
{
    ...
    public async Task<FileStream> Print(Guid LawId, Guid PhaseLawId, ConfImpVoteComission config, string outType, bool includeAmendmentRatraper)
    {
        ...
        var amendments = await _unitOfWork.Amendmnets.GetAmendmentsForVoteFileCommision(LawId, PhaseLawId, includeAmendmentRatraper);
        ...
        foreach (var amendment in amendments)
        {
            ...
            amendmentVMs_.Add(new AmendmentVM
            {
                ...
                NodeTitle = amendment.Node.Label!,
                NodeId = amendment.NodeId,
                NodeTypeId = amendment.Node.TypeId,
                teamTitle = amendment.Team.Name,
                type = amendment.AmendmentIntent!,
                number = amendment.Number,
                amendmentIntent = amendment.AmendmentIntent!,
                title = amendment.Title!,
                content = ManageEdition.UpdateStyle(amendment.Content!),
                ...
                ordre = amendment.Ordre,
                numberSystem = amendment.NumSystem,
                inFavor = vote?.InFavor,
                against = vote?.Against,
                titleParagraphe= $"{amendment.Title} - {amendment.TitreParagraphe} -",
                neutral = vote?.Neutral,
                result = Constants.GetVoteResult(vote?.Result ?? "")
            });
        }
        AmendmentVM[] amendmentVMs = amendmentVMs_.ToArray();
        if (amendmentVMs.Length > 0 && amendmentVMs != null)
        {
            ManageEdition.AddAmendGrpGuardPage(builder, LawTitle, string.Empty, true);
            builder.InsertBreak(BreakType.SectionBreakNewPage);
            foreach (var amendment in amendmentVMs)
            {
                await addAmendment(amendment, builder, config, true);
        ...
        {
            string ext = "pdf";
            var filename = "files/" + String.Format("VoteResult" + ext, "");
